...
export class StripeCustomerSyncService {
  ...
  async bulkSyncUsers(): Promise<{ synced: number; errors: number }> {
    try {
      ...
      console.log(`Found ${snapshot.size} clients to sync with Stripe`);
      ...
      for (const userDoc of snapshot.docs) {
        try {
          ...
          console.log(`Syncing user: ${user.profile?.firstName} ${user.profile?.lastName}`);
          ...
          console.log(`✅ Successfully synced user to Stripe: ${customer.id}`);
          synced++;
        } catch (error) {
